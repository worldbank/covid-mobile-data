...
__Timeframe__: Indicators are built from Feb 1, 2020 and going forward in real time.

__Data__: CDR or, if available, netwark probes.

__Description__:

Towers are clustered using Ward’s hierarchical clustering, with a maximum distance constraint of 1km from the centroid of the cluster. This results in ‘flat’ clusters. In our case, we used the clustering function: https://docs.scipy.org/doc/scipy-0.14.0/reference/generated/scipy.cluster.hierarchy.fcluster.html with criterion: distance and t = 1km. We then assign admin units based on the location of the centroid of the cluster (all observations in the cluster are then assigned to that admin unit). 
